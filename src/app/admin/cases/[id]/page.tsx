    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          culturalContextJson: JSON.stringify(data.culturalContextJson, null, 2),
          objectivesJson: JSON.stringify(data.objectivesJson, null, 2),
          learningObjectivesJson: JSON.stringify(data.learningObjectivesJson, null, 2),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Failed to update case');
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Modify the simulation scenario details
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl">
        ...
        {/* Personas Section */}
        {caseData.personas && caseData.personas.length > 0 && (
          <Card className="mt-6">
            <CardHeader>
              <CardTitle>Associated Personas</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {caseData.personas.map((persona) => (
                  <div key={persona.id} className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-gray-600">Avatar: {persona.avatarId}</p>
                    <p className="text-sm text-gray-600">Voice: {persona.voiceId}</p>
                  </div>
                ))}
              </div>
            </CardContent>
